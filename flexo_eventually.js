...
(function (flexo) {
  ...
  flexo.Promise = function () {
    this._queue = [];
    this._eventually_resolved = eventually_resolved_promise.bind(this);
  ...
  flexo.Promise.prototype = {
    ...
    eventually: function (on_fulfilled, on_rejected) {
      var p = new flexo.Promise;
      this._queue.push([p, on_fulfilled, on_rejected]);
      flexo.asap(this._eventually_resolved);
      return p;
    },
  ...
  function eventually_resolved_promise() {
    if (this.hasOwnProperty("value") || this.hasOwnProperty("reason")) {
      this.resolved_promise();
    } else {
      // TODO
    }
  }
    ...
    for (var i = 0; i < this._queue.length; ++i) {
      ...
      if (typeof p[on] == "function") {
        try {
          ...
          if (v && typeof v.eventually = "function") {
            v.eventually(function (value) {
              p[0].fulfill(value);
            }, function (reason) {
              p[0].reject(reason);
            });
